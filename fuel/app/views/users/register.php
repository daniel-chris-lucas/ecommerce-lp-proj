...
<?php echo Form::open() ?>
	...
	<div class="span6" style="margin-left: 0;">
		...
		<div class="control-group <?php if( $val->error( 'first_name' ) ) echo 'error' ?>" style="float: left; margin-right: 3px;">
			<input type="text" name="first_name" id="first_name" placeholder="First Name" value="<?php echo $val->validated( 'first_name' ) ?>">
		</div>
		<div class="control-group <?php if( $val->error( 'last_name' ) ) echo 'error' ?>">
			<input type="text" name="last_name" id="last_name" placeholder="Last Name" value="<?php echo $val->validated( 'last_name' ) ?>">
		</div>
		...
		<div class="control-group <?php if( $val->error( 'email' ) ) echo 'error' ?>" style="float: left; margin-right: 3px;">
			<input type="email" name="email" id="email" placeholder="E-Mail" value="<?php echo $val->validated( 'email' ) ?>">
		</div>
		<div class="control-group <?php if( $val->error( 'tel' ) ) echo 'error' ?>">
			<input type="text" name="tel" id="tel" placeholder="Telephone" value="<?php echo $val->validated( 'tel' ) ?>">
		</div>
		...
		<div class="control-group <?php if( $val->error( 'birth_day' ) || $val->error( 'birth_month' ) || $val->error( 'birth_year' ) ) echo 'error' ?>">
			...
			<select name="birth_day" id="birth_day" style="width: 105px;">
				...
				<?php foreach( $birth_days as $birth_day ) : ?>
					<option value="<?php echo $birth_day ?>" <?php if( $birth_day == $val->validated( 'birth_day' ) ) echo 'selected=selected'?>>
						<?php echo $birth_day ?>
					</option>
				<?php endforeach ?>
			</select>
			<select name="birth_month" id="birth_month" style="width: 125px;">
				...
				<?php foreach( $birth_months as $birth_month_k => $birth_month_v ) : ?>
					<option value="<?php echo $birth_month_k + 1 ?>"  <?php if( $birth_month_k + 1 == $val->validated( 'birth_month' ) ) echo 'selected=selected'?>>
			...
			<select name="birth_year" id="birth_year" style="width: 115px;">
				...
				<?php foreach( $birth_years as $birth_year ) : ?>
					<option value="<?php echo $birth_year ?>"  <?php if( $birth_year == $val->validated( 'birth_year' ) ) echo 'selected=selected'?>>
						<?php echo $birth_year ?>
					</option>
				<?php endforeach ?>
			</select>
	...
	<div class="span6">
		...
		<div class="control-group <?php if( $val->error( 'street' ) ) echo 'error' ?>" style="float: left; margin-right: 3px;">
			<input type="text" name="street" id="street" placeholder="Street Name" value="<?php echo $val->validated( 'street' ) ?>">
		</div>
		<div class="control-group <?php if( $val->error( 'street_number' ) ) echo 'error' ?>">
			<input type="text" name="street_number" id="street_number" placeholder="Street Number" value="<?php echo $val->validated( 'street_number' ) ?>">
		</div>
		...
		<div class="control-group <?php if( $val->error( 'town' ) ) echo 'error' ?>" style="float: left; margin-right: 3px;">
			<input type="text" name="town" id="town" placeholder="Town" value="<?php echo $val->validated( 'town' ) ?>">
		</div>
		<div class="control-group <?php if( $val->error( 'country' ) ) echo 'error' ?>">
			<select name="country" id="country">
				<option value="">---Country---</option>
				<?php foreach( $countries as $country ) : ?>
					<option value="<?php echo $country['id'] ?>" <?php if( $country['id'] == $val->validated( 'country' ) ) echo 'selected=selected' ?>>
	...
	<div class="span6" style="margin-left: 0;">
		...
		<div class="control-group <?php if( $val->error( 'username' ) ) echo 'error' ?>" style="float: left; margin-right: 3px;">
			<input type="text" name="username" id="username" placeholder="Username" value="<?php echo $val->validated( 'username' ) ?>">
		</div>
		...
		<div class="control-group <?php if( $val->error( 'password_confirm' ) ) echo 'error' ?>">
